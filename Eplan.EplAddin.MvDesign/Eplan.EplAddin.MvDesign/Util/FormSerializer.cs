    ...
    public static class FormSerializer
    {
        ...
        public static void Deserialise(Control c, string XmlFileName)
        {
            if (File.Exists(XmlFileName)) 
            {
                ...
                {
                    SetControlProperties((Control)c, node);
                }
            }
        }
        ...
        private static void AddChildControls(XmlTextWriter xmlSerialisedForm, Control c)
        {
            foreach (Control childCtrl in c.Controls)
            {
                if (!(childCtrl is Label))
                {
                    ...
                    else if (childCtrl is RichTextBox)
                    {
        ...
        private static void SetControlProperties(Control currentCtrl, XmlNode xmlNode)
        {
            // get the control's name and type
            string controlName = xmlNode.Attributes["Name"].Value;
            string controlType = xmlNode.Attributes["Type"].Value;
            // find the control
                    ...
                    if (ctrlToSet.GetType().ToString() == controlType)
                    {
                        ...
                        switch (controlType)
                        {
                            case "System.Windows.Forms.TextBox":
                                ((System.Windows.Forms.TextBox)ctrlToSet).Text = xmlNode["Text"].InnerText;
                                break;
                            case "System.Windows.Forms.RichTextBox":
                                ((System.Windows.Forms.RichTextBox)ctrlToSet).Text = xmlNode["Text"].InnerText;
                                break;
                            case "System.Windows.Forms.ComboBox":
                                ((System.Windows.Forms.ComboBox)ctrlToSet).Text = xmlNode["Text"].InnerText;
                                ((System.Windows.Forms.ComboBox)ctrlToSet).SelectedIndex = Convert.ToInt32(xmlNode["SelectedIndex"].InnerText);
                                break;
                            case "System.Windows.Forms.ListBox":
                                // need to account for multiply selected items
                                ListBox lst = (ListBox)ctrlToSet;
                                XmlNodeList xnlSelectedIndex = xmlNode.SelectNodes("SelectedIndex");
                            ...
                            case "System.Windows.Forms.CheckBox":
                                ((System.Windows.Forms.CheckBox)ctrlToSet).Checked = Convert.ToBoolean(xmlNode["Checked"].InnerText);
                                break;
                        }
                        ctrlToSet.Visible = Convert.ToBoolean(xmlNode["Visible"].InnerText);
                        // if n has any children that are controls, deserialise them as well
                        if (xmlNode.HasChildNodes && ctrlToSet.HasChildren)
                        {
                            XmlNodeList xnlControls = xmlNode.SelectNodes("Control");
